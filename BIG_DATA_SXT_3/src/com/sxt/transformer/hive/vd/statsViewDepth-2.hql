...
with serdeproperties(
    'hbase.columns.mapping'=':key,info:pl,info:en,info:s_time,info:p_url,info:u_ud,info:u_sd'
)
...
-- 6. 创建hive的function
create function platform_convert as 'com.sxt.transformer.hive.PlatformDimensionUDF' using jar 'hdfs://user/transformer/PlatformDimensionUDF.jar';
create function date_convert as 'com.sxt.transformer.hive.DateDimensionUDF' using jar 'hdfs://user/transformer/DateDimensionUDF.jar';
...
from (
    ...
    and pl is not null and s_time >= unix_timestamp('2019-09-16','yyyy-MM-dd')*1000
    and s_time < unix_timestamp('2019-09-17','yyyy-MM-dd')*1000
...
from tmp
insert overwrite table stats_view_depth select platform_convert(pl),date_convert(date1),5,sum(pv1),sum(pv2),sum(pv3),sum(pv4),sum(pv5_10),sum(pv10_30),sum(pv30_60),sum(pv60_plus),'2019-09-16' group by pl,date1;

-- 9. sqoop脚步编写(统计会话角度)
sqoop export --connect jdbc:mysql://centos:3306/result_db --username root --password 123456 --table stats_view_depth --export-dir /user/hive_remote/warehouse/stats_view_depth/* --input-fields-terminated-by "\\01" --update-mode allowinsert --update-key platform_dimension_id,data_dimension_id,kpi_dimension_id
