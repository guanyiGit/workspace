-- 1. 在hive中创建hbase的event_logs_2对应表
CREATE EXTERNAL TABLE event_log_2(
    key string,
    pl string,
    en string,
    s_time bigint,
...
'hbase.columns.mapping'=':key,info:pl,info:en,info:s_time,info:p_url,info:u_ud,info:u_sd,info:ca,info:ac,info:oid,info:on,info:cua,info:cut,info:pt'
)
...
-- 4. 创建function
create function currency_type_convert as 'com.sxt.transformer.hive.CurrencyTypeDimensionUDF' using jar 'hdfs://user/transformer/CurrencyTypeDimensionUDF.jar';
create function payment_type_convert as 'com.sxt.transformer.hive.PaymentTypeDimensionUDF' using jar 'hdfs://user/transformer/PaymentTypeDimensionUDF.jar';
create function order_info as 'com.sxt.transformer.hive.OrderInfoUDF' using jar 'hdfs://user/transformer/OrderInfoUDF.jar';
create function order_total_amount as 'com.sxt.transformer.hive.OrderTotalAmountUDF' using jar 'hdfs://user/transformer/OrderTotalAmountUDF';
...
或者
from (
    select oid,pl,s_time,cut,pt,cua from event_logs_2
    where en='e_crt' and pl is not null and s_time >= unix_timestamp('2015-12-13','yyyy-MM-dd')*1000 and s_time < unix_timestamp('2015-12-14','yyyy-MM-dd')*1000
) as tmp
insert overwrite table order_info select oid,pl,s_time,cut,pt,cua 
	sqoop脚本
sqoop export --connect jdbc:mysql://centos:3306/result_db --username root --password 123456 --table order_info --export-dir /user/hive_remote/warehouse/stats_view_depth/* --input-fields-terminated-by "\\01" --update-mode allowinsert --update-key order_id
...
from (
    ...
    where en='e_crt'
        and pl is not null
        and s_time >= unix_timestamp('2015-12-13','yyyy-MM-dd')*1000
        and s_time < unix_timestamp('2015-12-14','yyyy-MM-dd')*1000
...
-- 8. 订单数量sqoop(总的)
sqoop export --connect jdbc:mysql://centos:3306/result_db --username root --password 123456 --table stats_order --export-dir /user/hive_remote/warehouse/stats_view_depth/* --input-fields-terminated-by "\\01" --update-mode allowinsert --update-key platform_dimension_id,date_dimension_id,currency_type_dimension_id,payment_type_dimension_id --columns platform_dimension_id,date_dimension_id,currency_type_dimension_id,payment_type_dimension_id,orders,created


-- 9. 订单金额hql(总的)
from (
    from (
        select pl,from_unixtime(cast(s_time/1000 as bigint),'yyyy-MM-dd') as date,cut,pt,oid,max(cua) as amount
        from event_logs_2
        where en='e_crt'
            and pl is not null
            and s_time >= unix_timestamp('2015-12-13','yyyy-MM-dd')*1000
...
from (
    from (
        select order_info(oid,'pl') as pl,from_unixtime(cast(s_time/1000 as bigint),'yyyy-MM-dd') as date,order_info(oid,'cut') as cut,order_info(oid,'pt') as pt,oid,max(order_info(oid)) as amount
        from event_logs_2
        where en='e_cs'
            and pl is not null
            and s_time >= unix_timestamp('2015-12-13','yyyy-MM-dd')*1000
