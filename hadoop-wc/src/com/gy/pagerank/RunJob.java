...
public class RunJob {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            try {
                ...
                Path inputPath = new Path("hadoop-wc/data/pagerank/input/");

                if (i > 1) {
                    inputPath = new Path("hadoop-wc/data/pagerank/output/pr" + (i - 1));
                }
                FileInputFormat.addInputPath(job, inputPath);

                Path outpath = new Path("hadoop-wc/data/pagerank/output/pr" + i);
    ...
    static class PageRankReducer extends Reducer<Text, Text, Text, Text> {
        protected void reduce(Text key, Iterable<Text> iterable, Context context)
            ...
            double sum = 0.0;//单个入链的pr 即：出链对象总PR/出链对象总出链数

            Node sourceNode = null;
            for (Text i : iterable) {
                Node node = Node.fromMR(i.toString());
                if (node.containsAdjacentNodes()) {
                    sourceNode = node;//出链关系
                } else {
                    sum = sum + node.getPageRank();//所有对自己的投票 pr
                }
            }

            //PageRank算法公式 4为页面总数 0.85阻尼系数防止只入不出和只出不入
